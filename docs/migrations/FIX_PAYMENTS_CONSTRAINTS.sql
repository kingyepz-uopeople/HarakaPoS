-- =====================================================
-- FIX PAYMENTS TABLE CONSTRAINTS
-- This drops and recreates the payments/receipts tables
-- Run this if you're still getting "permission denied" errors
-- =====================================================

-- 1. Drop existing tables (if they have bad foreign keys)
DROP TABLE IF EXISTS receipts CASCADE;
DROP TABLE IF EXISTS payments CASCADE;

-- 2. Recreate payments table (NO foreign keys to auth.users)
CREATE TABLE payments (
  ...
  -- Tracking fields (NO FOREIGN KEY - just UUID)
  initiated_by UUID,
...
-- 3. Recreate receipts table (NO foreign keys to auth.users)
CREATE TABLE receipts (
  ...
  issued_by UUID, -- NO FOREIGN KEY - just UUID
...
CREATE INDEX idx_payments_mpesa_request ON payments(mpesa_request_id);
...
-- 6. Auto-generate receipt number trigger
...
-- 8. Auto-complete order on payment
...
-- 11. Create RLS policies for receipts
...
SELECT generate_receipt_number() as test_receipt_number;
